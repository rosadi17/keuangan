...
function get_list_sub_uraian(page, src, id) {
    $.ajax({
        url: '<?= base_url('masterdata/manage_sub_uraian') ?>/list/'+page,
...
function edit_sub_uraian(str) {
    ...
    $('#haribulan').val(arr[8]);
    $('#harga').val(arr[9]);
    $('#dialog_sub_uraian').dialog({ title: 'Edit sub_uraian' });
}
...
<button id="add_sub_uraian">Tambah Data</button>
<button id="reload_sub_uraian">Refresh</button>

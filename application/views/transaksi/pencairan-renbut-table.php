<table cellspacing="0" width="100%" class="list-data">
    ...
    <tr class="<?= ($key%2==1)?'even':'odd' ?> <?= ($data->nominal > $data->jml_renbut)?'warning':NULL ?>">
        ...
        <td class="aksi" align="center">
            <?php if ($status === 'Dicairkan') { ?>
            <a class='printing' onclick="cetak_bukti_kas('<?= $data->id_renbut ?>')" title="Klik untuk cetak bukti kas">&nbsp;</a>
            <?php } else { ?>
            <a class='pencairan' onclick="edit_pencairan('<?= $str ?>');" title="Klik untuk pencairan">&nbsp;</a>
            <?php } ?>
        </td>
    </tr>
    <?php } ?>
